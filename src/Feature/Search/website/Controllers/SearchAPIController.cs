namespace LionTrust.Feature.Search.Controllers
{
    ...
    public class SearchAPIController : SitecoreController
    {
        ...
        /// </summary>
        /// <returns>A list of articles.</returns>
        public ActionResult GetFilteredFunds(string fundTeams, string fundManagers, string fundRegions, string fundRanges, string searchTerm, string database = "web", int page = 1)
        {
            var response = this._fundListingDataManager.GetFundListingResponse(database, fundTeams, fundManagers, fundRegions, fundRanges, searchTerm, page);
            if (response.StatusCode != 200)
            {
                return new HttpStatusCodeResult(response.StatusCode, response.StatusMessage);
            }

namespace LionTrust.Feature.Listings.Controllers
{
    ...
    public class GenericListingApiController : SitecoreController
    {
        ...
        public ActionResult GetFilteredResults(string parentId, string type = "", List<int> months = null, List<int> years = null, string searchTerm = "", string database = "web", int page = 1)
        {
            var response = this._genericListingDataManager.GetGenericListingResponse(parentId, type, months?.ToList(), years?.ToList(), searchTerm, page, database);

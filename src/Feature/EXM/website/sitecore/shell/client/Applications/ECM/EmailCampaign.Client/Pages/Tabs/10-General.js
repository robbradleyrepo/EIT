define([
	...
	function generalTab_HideTeamDialog(contextApp) {
        var selectItem = contextApp.TeamTreeView.viewModel.getActiveNode();
        if (selectItem == null) {
            alert("Please, select team");
        } else if (selectItem.data.rawItem && selectItem.data.rawItem.$templateId == "{A87A00B1-E6DB-45AB-8B54-636FEC3B5523}") {
            alert("Please, select team, not folder");
        } else {
            var teamPath = selectItem.data.path.replace("/sitecore/content/LionTrustNew/Email Settings/Teams", "");
            contextApp.TeamTextBox.set("text", teamPath);
            contextApp.MessageContext.set("teamPath", teamPath);
            contextApp.MessageContext.set("team", selectItem.data.key);
            contextApp.TeamDialogWindow.hide();
        }
    }
    ...
    var generalTab = sitecore.Definitions.App.extend({
        initialized: function() {
			...
			this.on("select:campaign:team:browse", this.showTeamDialog, this);
            this.on("select:campaign:team:ok", this.hideTeamDialog, this);
            this.on("select:campaign:team:cancel", this.cancelTeamDialog, this);
		...
		showTeamDialog: function() {
            getActiveTreeViewItem(this.MessageContext.get("teamPath"), this.TeamTreeView, this.TeamDialogWindow);
        },
        hideTeamDialog: function() {
            generalTab_HideTeamDialog(this);
        },
        cancelTeamDialog: function() {
            this.TeamDialogWindow.hide();
        },
        updateTextBoxes: function(contextApp) {
			...
			setReadOnlyToTextBox(contextApp.TeamTextBox);
            contextApp.TeamTextBox.set("text", contextApp.MessageContext.get("teamPath"));

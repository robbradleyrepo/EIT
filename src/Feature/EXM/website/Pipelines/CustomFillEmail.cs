namespace LionTrust.Feature.EXM.Pipelines
{
    ...
    public class CustomFillEmail
    {
        ...
        public CustomFillEmail(IStringCipher cipher, ILogger logger, PipelineHelper pipelineHelper, EcmSettings settings, IFillEmailHelper fillEmailHelper)
        {
            _cipher = cipher;
            _logger = logger;
        ...
        public void Process(SendMessageArgs args)
        {
            ...
            if (args.EcmMessage is HtmlMailBase htmlMessage)
            {
                new HtmlMailFillerCustom(htmlMessage, args, _logger, _cipher, _pipelineHelper, _settings, _fillEmailHelper).FillEmail();

            }
            else if (args.EcmMessage is MailMessageItem mailMessage)
            {
                new MailMessageFillerCustom(mailMessage, args, _cipher, _logger, _pipelineHelper, _settings, _fillEmailHelper).FillEmail();
            }
        }
    }
    ...
    public class HtmlMailFillerCustom : HtmlMailFiller
    {
        ...
        public HtmlMailFillerCustom(
            HtmlMailBase message, 
            SendMessageArgs args, 
            ILogger logger, 
            IStringCipher cipher, 
            PipelineHelper pipelineHelper, 
            EcmSettings settings, 
            IFillEmailHelper fillMailHelper) :
    ...
    public class MailMessageFillerCustom : MailMessageFiller
    {
        ...
        public MailMessageFillerCustom(
            MailMessageItem message, 
            SendMessageArgs args, 
            IStringCipher cipher, 
            ILogger logger, 
            PipelineHelper pipelineHelper, 
            EcmSettings settings, 
            IFillEmailHelper fillEmailHelper) :

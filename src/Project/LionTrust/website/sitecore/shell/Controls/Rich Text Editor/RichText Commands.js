...
function scInsertSitecoreLink(sender, returnValue) {
  ...
  if (text == "" || text == null || ((text != null) && (text.length == 15) && (text.substring(2, 15).toLowerCase() == "<p>&nbsp;</p>"))) {
    ...
    if (match && match.length >= 2) {
	  //keep the 'class' attribute if it exists
	  if (d.className.length > 0){
		scEditor.pasteHtml("<p><a href=\"" + returnValue.url + "\" class=\"" + d.className + "\">" + match[1] + "</a></p>", "DocumentManager");
	  }
	  else{
  ...
  //keep the 'class' attribute if it exists
  if (d.className.length > 0){
	scEditor.pasteHtml("<a href=\"" + returnValue.url + "\" class=\"" + d.className + "\">" + text + "</a>", "DocumentManager");
  }
  else{

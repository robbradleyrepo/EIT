namespace LionTrust.Foundation.Contact.Services
{
    ...
    internal class SFEntityUtility
    {
        ...
        /// <param name=""></param>
        /// <returns></returns>
        public RegisterdUserReturnViewModel SaveNonProfUserAsSFLead(NonProfessionalUser nonProfUser)
        {
            try
            {
                ...
                if (results.success)
                {
                    ...
                    if (newlyCreatedLead != null)
                    {
                        ...
                        var editEmailPrefPageItem = Sitecore.Context.Database.GetItem(new ID(Constants.ItemIds.Content.Global.EmailPreferences.EditEmailPreferecesPage));
                        var editEmailPrefPagelink = string.Format("https://{0}{1}?{2}", HttpContext.Current.Request.Url.Host, LinkManager.GetItemUrl(editEmailPrefPageItem), queryStringParams);
        ...
        /// <returns></returns>
        public RegisterdUserReturnViewModel SaveProfUserAsSFContact(ProfessionalUser profUser)
        {
            try
            {
                var returnObj = new RegisterdUserReturnViewModel();
        ...
        /// <param name="isContact"></param>
        /// <returns></returns>
        public ResendEmailPrefUserDetails GetEmailDetailsForResendEmailPrefLink(string email, bool isContact)
        {
            try
            {
